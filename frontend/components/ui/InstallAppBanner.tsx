...
export default function InstallAppBanner() {
    ...
    useEffect(() => {
        ...
        const handleBeforeInstallPrompt = (e: Event) => {
            e.preventDefault();
            setDeferredPrompt(e);
            const dismissed = sessionStorage.getItem("neocinema_pwa_dismissed");
            if (!dismissed) {
                setShowBanner(true);
            }
        };
        ...
        // Custom event to force show banner from Navbar/Footer buttons
        const handleForceOpen = () => {
            setShowBanner(true);
            setShowGuide(false);
        };
        window.addEventListener("open-install-banner", handleForceOpen);

        // Show banner after 1.5s delay if not dismissed for current session
        ...
        return () => {
            ...
            window.removeEventListener("open-install-banner", handleForceOpen);
            clearTimeout(timer);
        };
    }, []);
    ...
    const handleDismiss = () => {
        setShowBanner(false);
        sessionStorage.setItem("neocinema_pwa_dismissed", "true");
    };
    ...
    return (
        <>
            {/* ─── Floating Bottom Shortcut Banner ──────────────────────────── */}
            {showBanner && !showGuide && (
                <div className="fixed bottom-4 left-4 right-4 z-[999] max-w-md mx-auto rounded-2xl bg-neutral-900/95 border border-red-500/40 p-4 shadow-[0_20px_50px_rgba(220,38,38,0.4)] backdrop-blur-2xl transition-all duration-500 animate-in slide-in-from-bottom-5">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-sm font-black text-white flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] font-black bg-red-600/30 text-red-400 border border-red-500/30 px-1.5 py-0.5 rounded-full uppercase">Shortcut</span>
                                </h3>
                        ...
                        <button
                            ...
                            className="text-neutral-400 hover:text-white p-1.5 rounded-lg hover:bg-white/10"
                    ...
                    <div className="mt-3.5 flex items-center gap-2">
                        ...
                        >
                            <Download size={15} />
                            <span>Add Shortcut To Phone / PC</span>
                        </button>
                        <button
                            onClick={handleDismiss}
            ...
            {/* ─── Manual Shortcut Guide Modal ──────────────────────── */}
            {showGuide && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                    <div className="w-full max-w-sm rounded-3xl bg-neutral-900 border border-white/10 p-6 shadow-2xl space-y-5 relative animate-in fade-in zoom-in-95">
                        <button
                            onClick={() => setShowGuide(false)}
                            className="absolute top-4 right-4 text-neutral-400 hover:text-white p-1.5 rounded-lg hover:bg-white/10"
                        >
                            <X size={20} />
                        </button>
                        ...
                        <div className="text-center space-y-2">
                            ...
                            <h3 className="text-lg font-black text-white">Add Neocinema Shortcut</h3>
                            <p className="text-xs text-neutral-400">Follow 2 fast steps to add the app icon directly to your home screen:</p>
                        </div>
                        ...
                        <div className="space-y-3 bg-white/5 rounded-2xl p-4 border border-white/5 text-xs text-neutral-300 font-medium">
                            <div className="flex items-start gap-3">
                                ...
                                <p>Tap browser menu <strong>(3 dots ⋮ in Chrome/Edge or Share ⎋ in Safari)</strong>.</p>
                            </div>
                            <div className="flex items-start gap-3">
